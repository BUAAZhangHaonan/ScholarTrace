#!/usr/bin/env python3
"""
Stress test: 20 concurrent MCP query requests via SSE.

Tests the multi-model concurrent pool:
  - Model priority: glm-5-turbo(5) → glm-4.7(20) → glm-4.6(20) → deepseek(10) → qwen(10)
  - Each request sends ~200 candidates, selects 25 papers via select_papers()
  - 20 concurrent requests saturate the pool, forcing overflow to fallback models
"""
...
TOPICS = [
    ...
    "contrastive learning for self-supervised representation learning",
    "transformer models for time series forecasting and anomaly detection",
    "large language model alignment and safety guardrails",
    "protein structure prediction with deep learning methods",
    "robotic manipulation learning from demonstration",
    "semantic segmentation in medical imaging with deep neural networks",
    "efficient fine-tuning methods for large pre-trained models",
    "continual learning without catastrophic forgetting",
    "generative adversarial networks for data augmentation",
    "speech recognition with end-to-end transformer architectures",
]
...
async def main():
    print("=" * 80)
    print("MULTI-MODEL POOL STRESS TEST")
    print("=" * 80)
    ...
    # Save raw results
    output_path = "docs/examples/results/stress_test_model_pool_result.json"
